...
export default class FileUtil {
    ...
    static zipDownload(files) {
        ...
        for (const name in files) {
            // zip.file(name, mapToJson(files[name]));
            zip.file(name, JSON.stringify(files[name], null, 4));
        }

        let time = new Date().toLocaleString(undefined,{hour12: false}).replace(/\/|:|\s/g,'-');
        // 把打包内容异步转成blob二进制格式
        zip.generateAsync({ type: "blob" }).then(function (content) {
            // content就是blob数据，这里以example.zip名称下载
            // 使用了FileSaver.js
            saveAs(content, `i18n${time}.zip`);
        });
    }
